...
做法：在 Hero 的技術標籤（tag）、設計重點、「專案實作項目」段落，**自然帶入具體名詞**（不是只寫「串接 API」這種空話）。
...
設計版面、排版節奏、配色搭配時可參考 [refero.design](https://styles.refero.design/)（介面設計靈感/風格庫），
抓的是版面編排手法與色彩搭配邏輯的「感覺」，不是照抄特定站的畫面或元件——最終仍要落在這個作品集既有的深色系 CSS 變數（`--bg`/`--accent`/`--card`/`--text` 等）之上，保持全站視覺一致。
...
5. **寫 README**（用上方範本），補「專案實作項目 + 設計決策」段落。
