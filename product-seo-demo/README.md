...
部落格內容生成、搜尋詞彙分類與加權再排序**；官方 **Shopify dev-mcp** 輔助開發查詢 schema，**宜搭 (Yida)** 表單則作為下架流程
...
- **搜尋詞彙分類與加權再排序**：同義詞庫／停用詞／單位正規化 + 標題 +20／供應商或類型 +10／Meta +3 加權重排序，含「優化前後效果對比」（示意：原生比對上千筆雜訊 → 優化後十餘筆精準結果）與權重圖例圖表
...
- 🔎 **搜尋詞彙分類＋加權再排序**：同義詞庫／停用詞／單位正規化改寫查詢，＋標題/供應商/Meta 加權重排序，示意可把上千筆雜訊結果收斂到十餘筆精準結果
